...
%       'data'                  The object can only be moved along 2d pair
%                               of x and y data.
%
...
%
%   'data'          [x1 x2 x3 x4 ... ;  Limits the drag range to the
%                    y1 y2 y3 y4 ... ]  specific coordinates in p.
%
%   -----------------------------------------------------------------------
%
% VERSION INFORMATION:
...
% 2020-04-13:   Probable fix for App Designer bug where current figure
%               handle was not properly retrieved. Thanks to William E.
%               Warriner for helping find a fix and testing it.
% 2023-03-27:   Using cleaned-up code from Gunnar Geissler which implements
%               the 'data' constraint.
%
...
if isempty(fgh) % ... But if it doesn't, let's try the parent...
    fgh = get(axh,'Parent');
end
if isempty(fgh) % Still empty. Let's find parents until a Figure is returned.
    curr_h = axh;
    while isempty(curr_h)
        curr_h = get(curr_h, 'Parent');
    end
fgh = curr_h;
end
...
set(h,'ButtonDownFcn',@(varargin)click_object(fgh,axh,varargin{:}));
...
% =========================================================================
function click_object(fgh,axh,obj,eventdata)
...
setappdata(obj,'initial_point',get(axh,'CurrentPoint'));
set(fgh,'WindowButtonDownFcn',{@(varargin)activate_movefcn(axh,varargin{:}),obj});
set(fgh,'WindowButtonUpFcn',{@deactivate_movefcn,obj});
activate_movefcn(axh,fgh,eventdata,obj);
...
% =========================================================================
function activate_movefcn(axh,obj,~,h)
% We were once setting up renderers here. Now we only set the movefcn
set(obj,'WindowButtonMotionFcn',{@(varargin)movefcn(axh,varargin{:}),h});
...
switch lower(constraint)
    ...
    case 'data'
        % using as outer range the axes limits
        range = [get(axh,'XLim') get(axh,'YLim')];
        x = p(1, :);
        y = p(2, :);

        % Forcing the object to remain on x/y data by going to the 
        % closest data point. Further down newpos is adjusted so that
        % it is exacly on x(datidx), y(dataidx).
        [~, dataidx] = min((cpt(1)-x).^2 + (cpt(2)-y).^2);
        dpt(1) = x(dataidx) - ipt(1);
        dpt(2) = y(dataidx) - ipt(2);
...
% Make sure marker is exactly on the data, even if the original marker was
% clicked a little bit off center, which has effects on the relative change
% dpt.
if strcmpi(constraint,'data')
    if size(newpos,1) == 1 % [x y [z / w h]]
        % We center x and y on the data point
        newpos(1) = x(dataidx);
        newpos(2) = y(dataidx);
    else                % [xdata; ydata]
        % We center the first point of the object on the data point
        newpos(1,1) = x(dataidx);
        newpos(2,1) = y(dataidx);
    end
